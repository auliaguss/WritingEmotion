...
| **Main Focus** | Swift's native Foundation Models framework generates prompts entirely on-device; drafts remain local and publishing goes through an injectable backend service (mocked until a base URL is configured). |
| **What we are skipping** | Reading other users' published posts. `readPublished(id:)` exists on the class diagram but is intentionally NOT implemented yet. |
...
| **16** | **Next up** | Real device testing of `FoundationModels` availability/perf and the daily-reset boundary (device timezone/midnight edge cases); configure and test the publishing backend URL; eventually connect `ReadView`/`readPublished(id:)`. | ⏳ In progress |
